              ...
              (= ",l" switch) (let [scm-prog (tools/load-prog-from-file (second input-commands))
                                ...
                                (if (nil? (:error last-eval))
                                  (do
                                    ...
                                    (println ";; Successfully loaded: " (second input-commands) "with result: "
                                             (tools/print-clj->scm (pr-str last-eval))))
                                  (println ";Error: " (tools/format-eval  (:error last-eval))" in Loading file " (second input-commands))))
              :else
              (when is-complete-sexp?
                (let [[new-env the-eval] (eval-exp-with-env @exec-env (tools/slurp-scm-str input))]
                  (if (nil? (:error (tools/format-eval the-eval)))
                    (do
                      (swap! exec-env merge new-env)
                      (println ";=> " (tools/format-eval  the-eval)))
                    (println ";Error: " (:error (tools/format-eval  the-eval)))))))
            (catch  #?(:clj Exception :cljs js/Error) e (println ";Error: " (pr-str e))))
